...
$cmd sentry.user sentry.organization sentry.organizationmember sentry.team sentry.project sentry.projectteam sentry.organizationmemberteam sentry.projectkey | grep -v "\[WARNING\]" > data/sentry_data.json
...
cat data/sentry_data.json|jq '[.[] | select(.model=="sentry.projectkey") | .model="projects.projectkey" | .fields.created=.fields.date_added | del(.fields.secret_key) | del(.fields.date_added) | del(.fields.roles) | del(.fields.status) ]' > data/projects_projectkey.json
